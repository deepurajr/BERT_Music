...
def create_spectrogram(args):
    mode = args.mode 
    if mode == "Train":
        if os.path.exists(root_dir+'Train_Spectogram_Images/'):
            return
        # Get Genres and Track IDs from the tracks.csv file
        filename_metadata = root_dir+"fma_metadata/tracks.csv"
        ...
        folder_sample = root_dir+"fma_small/"
        ...
        if not os.path.exists(root_dir+'Train_Spectogram_Images/'):
            os.makedirs(root_dir+'Train_Spectogram_Images/')
        for d in directories:
            ...
            for f in file_names:
                search_str = root_dir+'fma_small/.*/(.+?).mp3'
                f1 = f.replace("\\","/")
                search_res = re.search(search_str, f1)
                track_id = int(search_res.group(1))
                ...
                if(str(tracks_genre_array[track_index, 0]) != '0'):
                    ...
                    plt.savefig(root_dir+"Train_Spectogram_Images/"+str(counter)+"_"+str(tracks_genre_array[track_index,0])+".jpg", dpi=100)
                    plt.close()
                    counter = counter + 1
        return

    elif mode == "Test":
        if os.path.exists(root_dir+'Music_Spectogram_Images/'):
            return
        folder_sample = "../templates/music/"
        counter = 0
        print("Converting mp3 audio files into mel Spectograms ...")
        if not os.path.exists(root_dir+'Music_Sepctogram_Images/'):
            os.makedirs(root_dir+'Music_Spectogram_Images/')
        ...
        for f in file_names:
            ...
            plt.axes([0., 0., 1., 1.0], frameon=False, xticks=[], yticks=[])
            librosa.display.specshow(mel,cmap='gray_r')
            plt.savefig(root_dir+"Music_Spectogram_Images/"+test_id+".jpg", dpi=100)
            plt.close()
        return
