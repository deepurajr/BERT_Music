...
class RecommendModel:
    ...
    def __call__(self, *args, **kwargs):
        ...
        # Load model
        model_val.load_state_dict(torch.load(fld_path+'model_trial1.pt', map_location='cpu'))
        model_val.eval()
...
@RecommendModel
def load_data():
    filenames = [os.path.join(fld_path+"data/Music_Sliced_Images/", f) for f in os.listdir(fld_path+"data/Music_Sliced_Images/")
                    if f.endswith(".jpg")]
    images = []
    labels = []
    for f in filenames:
        srch_string = fld_path+'data/Music_Sliced_Images/.*_(.+?).jpg'
        #song_variable = re.search(r'data/Music_Sliced_Images/.*_(.+?).jpg', f).group(1)
        song_var = re.search(srch_string, f)
        song_variable = song_var.group(1)
